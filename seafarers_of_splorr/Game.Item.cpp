		...
		if (!initialized)
		{
			for (auto& item : ::data::Stores::GetStore(::data::Store::ITEM_DESCRIPTORS).items())
			{
				...
				descriptors.push_back(
				{
					...
					sfx,
					(descriptor.count(game::data::Properties::USE_STATISTIC) > 0) ? (std::optional<game::Statistic>((game::Statistic)(int)descriptor[game::data::Properties::USE_STATISTIC])) : (std::nullopt)
				});
			}
			initialized = true;
		}
