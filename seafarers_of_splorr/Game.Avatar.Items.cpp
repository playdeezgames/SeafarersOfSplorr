...
namespace game::avatar::Items
{
	...
	const size_t MapFragmentGenerationAttempts = 50;

	static common::XY<size_t> TryGenerateUnknownLocation(size_t fragmentWidth, size_t fragmentHeight)
	{
		...
		do
		{
			fragmentX = common::RNG::FromRange(0u, worldSize.GetX() - fragmentWidth);
			fragmentY = common::RNG::FromRange(0u, worldSize.GetY() - fragmentHeight);
			if (game::World::GetKnownState({ fragmentX, fragmentY }) == game::world::KnownState::UNKNOWN)
			{
				break;
			}
			attempt++;
		} while (attempt < MapFragmentGenerationAttempts);
		return { fragmentX, fragmentY };
	}
	...
	static void AddMapFragment()
	{
		...
		auto location = TryGenerateUnknownLocation(fragmentWidth, fragmentHeight);
	...
	void Add(int item, size_t amount)
	{
		if (amount > 0)
		{
			...
			else if (item == game::Avatar::GetDescriptor().wisdom)
			{
				//nothing happens.
			}
	...
	{
		auto descriptor = game::item::GetDescriptor(item);
		if (descriptor.useStatistic)
		{
			game::Statistics::Increment(descriptor.useStatistic.value());
		}
	...
	static std::optional<std::string> Eat(int item, bool combat)
	{
	...
	static std::optional<std::string> Heal(int item, bool combat)
	{
		return ConsumeItem(item, [](const game::item::Descriptor& descriptor)
		{
	...
	static 	std::optional<std::string> BuffAttack(int item, bool combat)
	{
		return ConsumeItem(item, [](const game::item::Descriptor& descriptor)
		{
	...
	static std::optional<std::string> BuffDefend(int item, bool combat)
	{
		return ConsumeItem(item, [](const game::item::Descriptor& descriptor)
		{
	...
	static std::optional<std::string> UseSoap(int item, bool combat)
	{
		return ConsumeItem(item, [](const game::item::Descriptor& descriptor)
		{
	...
	static std::optional<std::string> Teleport(int item, bool combat)
	{
		return ConsumeItem(item, [](const game::item::Descriptor& descriptor)
		{
			auto worldSize = game::World::GetSize();
			...
			game::Avatar::SetTeleportFlag();
			return ToConsumeItemResult(descriptor);
		}, ResultToSfx, combat);
	}

	static std::map<game::item::Usage, std::function<std::optional<std::string>(int, bool)>> nonCombatVerbs =
	{
		...
		{game::item::Usage::BRIBE, [](int, bool) { return std::nullopt; }},
		{game::item::Usage::ATTITUDE, [](int, bool) { return std::nullopt; }},
		{game::item::Usage::JOKER, [](int, bool) { return std::nullopt; }},
	...
	std::optional<std::string> Use(std::optional<int> item)
	{
		if (item)
		{
			...
			if (usage)
			{
				return nonCombatVerbs.find(*usage)->second(*item, false);
			}
		}
		return std::nullopt;
	}
	...
	{
		auto result = Teleport(item, true);
		if (result)
		{
	...
	{
		auto result = BuffAttack(item, true);
		if (result)
		{
			return std::make_tuple(*result, CombatUseResult::SKIP);
		}
	...
	{
		auto result = BuffDefend(item, true);
		if (result)
		{
			return std::make_tuple(*result, CombatUseResult::SKIP);
		}
