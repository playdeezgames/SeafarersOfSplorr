...
namespace game::world::Borders
{
	...
	static world::Border GetNSBorder(size_t index)
	{
		...
		return world::Border::UNBREAKABLE_WALL;
	}
	...
	static WallBumpResult BumpBorder(nlohmann::json& bumps, size_t index, const game::world::Border& border)
	{
		if (border == game::world::Border::BREAKABLE_WALL)
		{
			auto key = IndexToString(index);
			if (bumps.count(key) > 0)
			{
				bumps[key] = bumps[key] + 1;
			}
			else
			{
				bumps[key] = -9;//TODO: this should be part of the difficulty setting
			}
			int bumpCount = bumps[key];
			if (common::RNG::FromRange(0, 20) < bumpCount)
			{
				return WallBumpResult::WALL_FALLS;
			}
		}
		return WallBumpResult::JUST_PAIN;
	}

	static WallBumpResult BumpNSBorder(size_t index, const game::world::Border& border)
	{
		return BumpBorder(game::world::Data::GetNSBumps(), index, border);
	}

	static WallBumpResult BumpEWBorder(size_t index, const game::world::Border& border)
	{
		return BumpBorder(game::world::Data::GetEWBumps(), index, border);
	}

	WallBumpResult BumpNorthBorder(const common::XY<size_t>& position, const game::world::Border& border)
	{
		return BumpNSBorder(indexPlotters.find(IndexPlotter::NORTH)->second(position), border);
	}

	WallBumpResult BumpSouthBorder(const common::XY<size_t>& position, const game::world::Border& border)
	{
		return BumpNSBorder(indexPlotters.find(IndexPlotter::SOUTH)->second(position), border);
	}

	WallBumpResult BumpEastBorder(const common::XY<size_t>& position, const game::world::Border& border)
	{
		return BumpEWBorder(indexPlotters.find(IndexPlotter::EAST)->second(position), border);
	}

	WallBumpResult BumpWestBorder(const common::XY<size_t>& position, const game::world::Border& border)
	{
		return BumpEWBorder(indexPlotters.find(IndexPlotter::WEST)->second(position), border);
	}

	const std::map<maze::Direction, std::function<WallBumpResult(const common::XY<size_t>&, const game::world::Border&)>> borderBumpers =
	{
		{maze::Direction::NORTH, BumpNorthBorder},
		{maze::Direction::EAST, BumpEastBorder},
		{maze::Direction::SOUTH, BumpSouthBorder},
		{maze::Direction::WEST, BumpWestBorder},
	};

	WallBumpResult BumpAhead(const common::XY<size_t>& position, const maze::Direction& direction)
	{
		auto border = GetBorderAhead(position, direction);
		return borderBumpers.find(direction)->second(position, border);
	}
	...
	static world::Border DetermineBorder(const std::shared_ptr<maze::Cell<maze::Direction, maze::Door>>& cell, const maze::Direction& direction)
	{
		...
		else if(mazeDoor)//the border exists
		{
			auto nextCell = cell->GetNeighbor(direction);
			if (IsCellDeadEnd(cell) || IsCellDeadEnd(nextCell.value()))
			{
			...
			{
				return world::Border::BREAKABLE_WALL;
			}
		}
		else//the border is on the edge of the map
		{
			return world::Border::UNBREAKABLE_WALL;
		}
	}
