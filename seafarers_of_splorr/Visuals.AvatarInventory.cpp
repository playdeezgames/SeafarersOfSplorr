...
namespace visuals::AvatarInventory
{
	...
	static void DrawInternalAvatarInventory(const std::shared_ptr<common::Application::Renderer>& renderer, size_t avatarInventoryIndex)
	{
		...
		{
			std::string ss = std::format("{} x {}", game::item::GetDescriptor(entry.first).name, entry.second);
			...
			{
				visuals::Fonts::WriteText(avatarInventory.font, renderer, xy + avatarInventory.dropShadowXY, ss, avatarInventory.dropShadowColor, visuals::HorizontalAlignment::LEFT);
			}
			visuals::Fonts::WriteText(avatarInventory.font, renderer, xy, ss, color, visuals::HorizontalAlignment::LEFT);
