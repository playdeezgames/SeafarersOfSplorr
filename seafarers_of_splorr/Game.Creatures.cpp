...
namespace game::Creatures
{
	...
	{
		return std::format("({},{})", location.GetX(), location.GetY());
	}
	...
	std::optional<std::string> ChangeAttitude(const common::XY<size_t>& location, int item)
	{
		...
		if (instance)
		{
			...
			{
				if (iter->second.achievement.has_value())
				{
					game::Achievements::Add(iter->second.achievement.value());
				}
				if (iter->second.statistic.has_value())
				{
					game::Statistics::Increment(iter->second.statistic.value());
				}
