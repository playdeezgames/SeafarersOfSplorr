	...
	static void Add(nlohmann::json& creatureDescriptor)
	{
		...
		if (creatureDescriptor.count(game::data::Properties::ATTITUDES) > 0)
		{
			...
			for (auto& amount : amounts.items())
			{
				...
				auto achievement = (item.count(game::data::Properties::ACHIEVEMENT) > 0) ? (std::optional<game::Achievement>((game::Achievement)(int)item[game::data::Properties::ACHIEVEMENT])) : (std::nullopt);
				auto statistic = (item.count(game::data::Properties::STATISTIC) > 0) ? (std::optional<game::Statistic>((game::Statistic)(int)item[game::data::Properties::STATISTIC])) : (std::nullopt);
		...
		descriptors.push_back(
		{
			...
			(game::Statistic)(int)creatureDescriptor[game::data::Properties::KILLED_BY_STATISTIC],
			(game::Achievement)(int)creatureDescriptor[game::data::Properties::DEATH_BY_ACHIEVEMENT]
		});
	}
